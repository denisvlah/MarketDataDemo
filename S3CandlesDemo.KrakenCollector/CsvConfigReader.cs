...
/// </summary>
public static class CsvConfigReader
{
    private static readonly HashSet<int> ValidIntervals = new() { 1, 5, 15, 30, 60, 240, 1440, 10080, 21600 };

    public static List<CollectorJobConfig> ReadFromFile(string filePath)
    {
    ...
    /// <summary>
    /// Downloads the config CSV from S3 and parses it.
    /// </summary>
    public static async Task<List<CollectorJobConfig>> ReadFromS3Async(
    ...
    public static List<CollectorJobConfig> ParseLines(IEnumerable<string> lines)
    {
        var result = new List<CollectorJobConfig>();
        int lineNum = 0;
        ...
        foreach (var rawLine in lines)
        {
            ...
            result.Add(new CollectorJobConfig(assetPair, krakenPair, interval, startDate));
        }
