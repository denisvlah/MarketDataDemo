# MarketDataDemo.KrakenHistoricalImporter

Downloads all quarterly OHLCVT ZIP archives from [Kraken's Google Drive folder](https://drive.google.com/drive/folders/15RSlNuW_h0kVM8or8McOGOMfHeBFvFGI), extracts the CSV files, renames them to match the `MarketDataDemo.CsvLoader` naming convention, and uploads them to the S3 `csv/` folder.

> **Note:** After uploading, run `MarketDataDemo.CsvLoader` to convert the CSVs into the binary candle format consumed by the API.
...
The `MarketDataDemo.CsvLoader` expects files named:
...
# Run the importer
dotnet run --project MarketDataDemo.KrakenHistoricalImporter
